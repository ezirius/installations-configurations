    ...
    case "$2" in
      inspect)
        if [[ -f "$STATE_DIR/after-update" ]]; then
          printf '[{"State":"running","cpus":4}]\n'
        else
          printf '[{"State":"running","cpus":2}]\n'
        fi
        ;;
      set)
        if [[ "${3:-}" == --help ]]; then
...
assert_contains "$STATE_DIR/podman.log" 'machine set --memory 8192 podman-machine-default' 'supported podman memory option is applied'
assert_contains "$STATE_DIR/podman.log" 'machine set --disk-size 60 podman-machine-default' 'supported podman disk option is applied'
assert_contains "$STATE_DIR/podman.log" 'machine set --rootful false podman-machine-default' 'supported podman rootful option is applied'
...
case "$1" in
  machine)
    case "$2" in
      inspect)
        printf '[{"State":{"Running":false},"cpus":2}]\n'
        ;;
      set)
        if [[ "${3:-}" == --help ]]; then
          printf '%s\n' '--cpus --memory'
...
EOF
chmod +x "$MOCK_BIN/podman"
if PATH="$MOCK_BIN:$PATH" HOME="$HOME_DIR" XDG_CONFIG_HOME="$HOME_DIR/.config" STATE_DIR="$STATE_DIR" "$SCRIPT_FILE" >/dev/null 2>"$STATE_DIR/unsupported.err"; then
  printf 'assertion failed: podman-configure should fail when a required machine set flag is unsupported\n' >&2
  exit 1
fi
assert_contains "$STATE_DIR/unsupported.err" 'does not support required machine setting' 'podman-configure reports unsupported required settings clearly'
