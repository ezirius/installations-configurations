...
SCRIPT_FILE="$REPO_DIR/scripts/podman/macos/podman-check"

mkdir -p "$MOCK_BIN" "$STATE_DIR" "$HOME_DIR/Documents/Ezirius/Systems/Installations and Configurations/Computers" "$REPO_DIR/scripts/podman/macos" "$REPO_DIR/lib/shell" "$REPO_DIR/lib/shell/shared" "$REPO_DIR/config/podman/macos" "$REPO_DIR/config/repo/shared"
trap 'rm -rf "$TMPDIR" /tmp/podman-diagnose-absolute' EXIT
source "$HELPERS"

cp "$ROOT/scripts/podman/macos/podman-check" "$REPO_DIR/scripts/podman/macos/podman-check"
...
cat > "$REPO_DIR/config/podman/macos/podman-runtime-settings-shared.conf" <<'EOF'
