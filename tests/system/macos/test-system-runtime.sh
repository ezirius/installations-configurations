...
assert_contains "$STATE_DIR/defaults.log" 'write com.apple.dock mru-spaces -bool true' 'host system config is used instead of shared fallback when present'
...
if PATH="$MOCK_BIN:$PATH" HOME="$HOME_DIR" STATE_DIR="$STATE_DIR_INCOMPLETE" DOCK_AUTOHIDE_STATE=1 DOCK_MRU_STATE=0 "$REPO_DIR/scripts/system/macos/system-configure" >/dev/null 2>"$STATE_DIR_INCOMPLETE/err"; then
  printf 'assertion failed: system-configure should reject incomplete host-specific configs instead of layering shared defaults\n' >&2
  exit 1
fi
assert_contains "$STATE_DIR_INCOMPLETE/err" 'DOCK_REORDER_SPACES_BY_RECENT_USE is not set in the managed macOS system config' 'host-specific system configs must be self-contained when selected'
...
if PATH="$MOCK_BIN:$PATH" HOME="$HOME_DIR" STATE_DIR="$STATE_DIR_MISSING_METADATA" DOCK_AUTOHIDE_STATE=1 DOCK_MRU_STATE=0 "$REPO_DIR/scripts/system/macos/system-configure" >/dev/null 2>"$STATE_DIR_MISSING_METADATA/err"; then
  printf 'assertion failed: system-configure should reject host-specific configs missing required metadata\n' >&2
  exit 1
fi
assert_contains "$STATE_DIR_MISSING_METADATA/err" 'SYSTEM_DOCK_DOMAIN is not set in the managed macOS system config' 'system-configure reports missing host-selected metadata clearly'
...
PATH="$MOCK_BIN:$PATH" HOME="$HOME_DIR" STATE_DIR="$STATE_DIR_SHARED" DOCK_AUTOHIDE_STATE=0 DOCK_MRU_STATE=1 "$REPO_DIR/scripts/system/macos/system-configure" >/dev/null
assert_contains "$STATE_DIR_SHARED/defaults.log" 'write com.apple.dock autohide -bool true' 'system-configure falls back to the shared config when no host-specific file exists'
assert_contains "$STATE_DIR_SHARED/defaults.log" 'write com.apple.dock mru-spaces -bool false' 'shared fallback still configures dock space rearranging'
assert_contains "$STATE_DIR_SHARED/pmset.log" '-c sleep 0' 'shared fallback still configures portable pmset sleep'
