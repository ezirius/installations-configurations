...
EOF
cat > "$REPO_DIR/scripts/brew/macos/host-bootstrap" <<'EOF'
#!/usr/bin/env bash
...
printf 'Darwin\n'
EOF
cat > "$TMPDIR/scutil" <<'EOF'
#!/usr/bin/env bash
printf 'Maldoria\n'
EOF
chmod +x "$TMPDIR/uname" "$TMPDIR/scutil"
...
PATH="$TMPDIR:$PATH" BOOTSTRAP_RUNTIME_LOG="$LOG_FILE" "$REPO_DIR/scripts/brew/macos/brew-bootstrap" >/dev/null
ACTUAL="$(cat "$LOG_FILE")"
if [[ "$ACTUAL" != $'host-bootstrap\nbrew-service restart' ]]; then
  printf 'assertion failed: brew-bootstrap should work with a matching host-specific workflow config only\nactual:\n%s\n' "$ACTUAL" >&2
  exit 1
fi
