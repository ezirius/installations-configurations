...
SCRIPT_FILE="$REPO_DIR/scripts/brew/macos/brew-upgrade"
HOST_PYTHON3="$(command -v python3)"
mkdir -p "$MOCK_BIN" "$HOME_DIR/Documents/Ezirius/Systems/Installations and Configurations/Computers" "$BREW_PREFIX" "$REPO_DIR/scripts/brew/macos" "$REPO_DIR/lib/shell" "$REPO_DIR/lib/shell/shared" "$REPO_DIR/config/brew/macos" "$REPO_DIR/config/repo/shared" "$REPO_DIR/config/podman/macos"
trap 'rm -rf "$TMPDIR"' EXIT
source "$HELPERS"

cp "$ROOT/scripts/brew/macos/brew-upgrade" "$REPO_DIR/scripts/brew/macos/brew-upgrade"
...
assert_contains "$STATE_DIR/out" "Checking Brewfile: $REPO_DIR/config/brew/macos/brew-packages-shared.Brewfile" 'brew-upgrade processes the shared Brewfile by default'
assert_not_contains "$STATE_DIR/out" "Checking Brewfile: $REPO_DIR/config/brew/macos/brew-packages-maldoria.Brewfile" 'brew-upgrade does not report a host-specific Brewfile when none is present'
...
HOST_BREWFILE="$REPO_DIR/config/brew/macos/brew-packages-maldoria.Brewfile"
cat > "$HOST_BREWFILE" <<'EOF'
brew "podman"
EOF
git -C "$REPO_DIR" add config/brew/macos/brew-packages-maldoria.Brewfile >/dev/null
git -C "$REPO_DIR" commit -m 'Add host brewfile for runtime test' >/dev/null
git -C "$REPO_DIR" push >/dev/null 2>&1
STATE_DIR="$TMPDIR/state-host-fallback"
mkdir -p "$STATE_DIR"
...
PATH="$MOCK_BIN:$PATH" HOME="$HOME_DIR" STATE_DIR="$STATE_DIR" "$SCRIPT_FILE" >"$STATE_DIR/out"
assert_not_contains "$STATE_DIR/out" "Checking Brewfile: $REPO_DIR/config/brew/macos/brew-packages-shared.Brewfile" 'brew-upgrade skips the shared Brewfile when a host-specific Brewfile exists'
assert_contains "$STATE_DIR/out" "Checking Brewfile: $REPO_DIR/config/brew/macos/brew-packages-maldoria.Brewfile" 'brew-upgrade prefers the host-specific Brewfile when present'
...
assert_not_contains "$STATE_DIR/brew.log" 'upgrade caddy' 'brew-upgrade does not process shared Brewfile entries when a host-specific Brewfile exists'
