...
SCRIPT_FILE="$REPO_DIR/scripts/brew/macos/brew-install"
HELPERS="$ROOT/tests/shared/shared/runtime-helpers.sh"
HOST_PYTHON3="$(command -v python3)"
mkdir -p "$MOCK_BIN" "$NO_BREW_BIN" "$REPO_DIR/scripts/brew/macos" "$REPO_DIR/lib/shell" "$REPO_DIR/lib/shell/shared" "$REPO_DIR/config/brew/macos" "$REPO_DIR/config/repo/shared" "$REPO_DIR/config/podman/macos"
...
assert_contains "$STATE_DIR/out" "Checking Brewfile: $REPO_DIR/config/brew/macos/brew-packages-shared.Brewfile" 'brew-install processes the shared Brewfile by default'
...
STATE_DIR="$TMPDIR/state-default-host-fallback"
mkdir -p "$STATE_DIR"
touch "$STATE_DIR/clt.installed" "$STATE_DIR/installed-formula-caddy" "$STATE_DIR/installed-cask-ghostty"
PATH="$TMPDIR/brew-present:$MOCK_BIN:$PATH" STATE_DIR="$STATE_DIR" "$SCRIPT_FILE" >"$STATE_DIR/out" 2>"$STATE_DIR/err"
assert_not_contains "$STATE_DIR/out" "Checking Brewfile: $REPO_DIR/config/brew/macos/brew-packages-shared.Brewfile" 'brew-install skips the shared Brewfile when a host-specific Brewfile exists'
assert_contains "$STATE_DIR/out" "Checking Brewfile: $REPO_DIR/config/brew/macos/brew-packages-maldoria.Brewfile" 'brew-install prefers the host-specific Brewfile when present'
